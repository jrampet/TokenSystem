...
<?php 
if (!isset($_POST['sAcc'])){
$GLOBALS['username']=$_GET['user']; 
$GLOBALS['did'] = $_GET['did'];
}
else{
...
?>
				<ul>
                    <li style="float:left;border-right:none"><strong><?php 
                    echo $username; ?></strong></li>
...
<form action="viewpatient.php" method="post">
	...
	$sql1 = "Select * from book where DID ='".$did."' order by DOV desc";
				...
				while($row2= mysqli_fetch_array($result2))
				{
                    $sql3="SELECT * from clinic where CID=".$row1['CID'];
						...
						while($row3= mysqli_fetch_array($result3))
						{
                            ...
                            echo '<input type="hidden" name="dname" value='.$username.'>';
								echo "<tr>";
								echo "<td>" . $row1['DOV'] . "</td>";
                                ...
                                echo "<td>" . $row1['Status'].
								"<button type='submit' name='sAcc' value='{$bid}'>Accept</button>".
								"<button type='submit' name='sRej' value='{$bid}'>Reject</button></td>";
            ...
            if (isset($_POST['sAcc']) ) {
				$booking_id = (int) $_POST['sAcc'];
				
					$status="Accepted";
					//echo $status;
					$sql = "UPDATE book SET Status='".$status."' WHERE BID=$booking_id";
